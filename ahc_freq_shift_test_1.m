...
%反馈衰减
feed_gain = 0.2;  %如果这个反馈衰减值不够的话，就无法抑制啸叫
%移频处理参数
shift_freq = 3; %移动频率3hz
% 扩音系统内部传递路径c 注意这是个列向量，卷积计算时，直接矩阵相乘就行
c = [0,0,0,0,1]'; 
...
for i = 1:data_num
    ...
    shift_sig_out(i) = shift_sig_out(i) * exp(2*pi*1i*i/fs*shift_freq); % 频移f_shift
...
xs6 = zeros(1, length(c)); %中间计算缓存
temp = 0;
for i = 1:data_num
    %计算反馈输入信号
    xs6 = [sig_in(i) + temp, xs6(1:end-1)];   
    feed_sig_in(i) = feed_gain * (xs6 * c);  
    %移频处理
    xs4 = [feed_sig_in(i), xs4(1:end-1)]; %将输入信号加入卷积输入缓存区
    anti_feed_sig_out(i) = xs4 * h; %卷积处理。因为时域卷积等于频域乘积，所以这里相当于频率移动了。
    anti_feed_sig_out(i) = anti_feed_sig_out(i) * exp(2*pi*1i*i/fs*shift_freq); % 频移shift_freq
